...
const InvoiceCreate = () => {
  ...
  const searchClient = (value) => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        // NOTE: the new API only exposes GET /clients/{clientId} (lookup by ID),
        // not a name search. Until a search endpoint exists, this calls the same
        // path as a best-effort lookup and will usually return nothing useful.
